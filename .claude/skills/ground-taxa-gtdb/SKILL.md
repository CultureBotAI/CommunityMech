...
NCBITaxon id** — *Bacteria* → `d__Bacteria` is a tautology.

**Filter them out before quoting a coverage figure.** The rank is the letter
before `__` in `gtdb_id`, so this is a query rather than a substring search of
prose:
...
- **`is_reclassified: true`** — GTDB uses a different name than NCBI at that
  rank. Read it as exactly that and no more: 51 of 137 true values (37%) are a
