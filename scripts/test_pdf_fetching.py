...
class PDFFetchingTester:
    ...
    def test_single_doi(
        self, doi: str, use_fallback: bool = False
    ...
    def test_cascade(self, dois: list[str], use_fallback: bool = True):
        ...
        for i, doi in enumerate(dois, 1):
            ...
            if source:
                self.results[source].append({"doi": doi, "pdf_url": pdf_url})
    ...
    def test_html_parsing(self):
        ...
        for i, html in enumerate(test_cases, 1):
            ...
            if pdf_url:
                ...
                print("  ✗ Failed to extract PDF URL")
    ...
    def generate_report(self):
        ...
        print(
            f"Failed: {len(self.results['failed'])} ({100*len(self.results['failed'])/total_tested if total_tested > 0 else 0:.1f}%)"
        )
        ...
        for tier in tier_order:
            ...
            print(
                f"{status} Tier {tier_order.index(tier)+1} ({tier:20s}): {count:3d} ({pct:5.1f}%)"
            )
    ...
    def test_specific_publishers(self):
        ...
        for publisher, doi_prefix in test_dois.items():
            ...
            has_pattern = (
                self.fetcher_with_fallback._get_pdf_url_from_publisher(doi_prefix) is not None
            )
...
def main():
    ...
    success_rate = (
        100
        * sum(len(items) for source, items in tester.results.items() if source != "failed")
        / len(dois)
    )
