...
class NCBIValidation:
    ...
    is_match: bool
    ncbi_current_label: str | None
    suggested_ncbi_id: str | None
    suggested_ncbi_label: str | None
    is_synonym: bool = False
    synonym_type: str | None = None
    taxonomy_rank: str | None = None

    # Strain-level fields
    strain_level_id: str | None = None
    strain_level_label: str | None = None
    species_level_id: str | None = None
    species_level_label: str | None = None
    recommendation: str | None = None  # USE_STRAIN, USE_SPECIES, NOT_FOUND
...
def extract_strain_info(preferred_term: str, notes: str = "") -> StrainInfo:
    ...
    return StrainInfo(
        species_name=preferred_term.strip(), strain_designation=None, original_term=preferred_term
    )
...
def collect_all_taxa(kb_dir: Path) -> list[TaxonInfo]:
    ...
    if not communities_dir.exists():
        print(
            f"{Colors.RED}Error: Communities directory not found: {communities_dir}{Colors.RESET}"
        )
        return all_taxa
...
def search_ncbi_taxonomy_multilevel(
    species_name: str, strain: str | None, ncbi_adapter
...
def validate_ncbi_taxonomy(taxon: TaxonInfo, ncbi_adapter) -> NCBIValidation:
    ...
    try:
        ...
        strain_result, species_result = search_ncbi_taxonomy_multilevel(
            strain_info.species_name, strain_info.strain_designation, ncbi_adapter
        )
...
def query_gtdb_api(species_name: str) -> GTDBInfo:
    ...
    try:
        ...
        url = "https://api.gtdb.ecogenomic.org/search/taxa"
        ...
        if response.status_code == 200:
            ...
            if data and len(data) > 0:
                ...
                return GTDBInfo(found=True, lineage=lineage, classification=classification)
...
def load_gtdb_local(gtdb_path: Path | None) -> dict | None:
...
def compare_classifications(
    ncbi_validation: NCBIValidation, gtdb_info: GTDBInfo
) -> tuple[bool, str | None]:
...
def generate_report(
    ...
    total = (
        len(perfect_matches)
        + len(species_fallbacks)
        + len(wrong_level)
        + len(strain_not_in_db)
        + len(mismatches)
    )
    ...
    print(
        f"Perfect strain-level matches: {len(perfect_matches)} ({len(perfect_matches)/total*100:.1f}%)"
        if total > 0
        else "Perfect matches: 0"
    )
    print(
        f"Species-level fallback (acceptable): {len(species_fallbacks)} ({len(species_fallbacks)/total*100:.1f}%)"
        if total > 0
        else "Species fallback: 0"
    )
    print(
        f"Wrong taxonomy level: {len(wrong_level)} ({len(wrong_level)/total*100:.1f}%)"
        if total > 0
        else "Wrong level: 0"
    )
    print(
        f"Strain not in databases: {len(strain_not_in_db)} ({len(strain_not_in_db)/total*100:.1f}%)"
        if total > 0
        else "Strain not in DB: 0"
    )
    print(
        f"NCBITaxon ID mismatches: {len(mismatches)} ({len(mismatches)/total*100:.1f}%)"
        if total > 0
        else "Mismatches: 0"
    )
    ...
    print(f"{'-'*80}")
    print(
        f"Total taxa with strain designation: {strain_stats['with_strain']} ({strain_stats['with_strain']/total*100:.1f}%)"
        if total > 0
        else "With strain: 0"
    )
    print(
        f"  - Strain-specific NCBITaxon ID available: {strain_stats['strain_in_ncbi']} ({strain_stats['strain_in_ncbi']/strain_stats['with_strain']*100:.1f}%)"
        if strain_stats["with_strain"] > 0
        else "  - Strain in NCBI: 0"
    )
    print(
        f"  - Species-level fallback used: {strain_stats['species_fallback']} ({strain_stats['species_fallback']/strain_stats['with_strain']*100:.1f}%)"
        if strain_stats["with_strain"] > 0
        else "  - Species fallback: 0"
    )
    ...
    print()
    print(
        f"Total taxa without strain (species-level): {strain_stats['without_strain']} ({strain_stats['without_strain']/total*100:.1f}%)"
        if total > 0
        else "Without strain: 0"
    )
    print()
    ...
    if perfect_matches:
        print(
            f"{Colors.BOLD}{Colors.GREEN}A. PERFECT STRAIN-LEVEL MATCHES ({len(perfect_matches)}){Colors.RESET}"
        )
        print(f"{'-'*80}")
        for taxon, ncbi_val, gtdb_info in perfect_matches[:5]:  # Show first 5
            ...
            print(
                f'  Current NCBITaxon:{taxon.ncbi_id} → "{ncbi_val.ncbi_current_label}" {Colors.GREEN}✓{Colors.RESET}'
            )
            if gtdb_info.found and gtdb_info.lineage:
                print(f"  GTDB: {gtdb_info.lineage}")
            print("  STATUS: CORRECT - Strain-specific ID used")
            print()
    ...
    if species_fallbacks:
        print(
            f"{Colors.BOLD}{Colors.YELLOW}B. SPECIES-LEVEL FALLBACK (Acceptable) ({len(species_fallbacks)}){Colors.RESET}"
        )
        print(f"{'-'*80}")
        for taxon, ncbi_val, gtdb_info in species_fallbacks:
            ...
            print(
                f'  Literature: "{taxon.preferred_term}" (strain specified: {strain_info.strain_designation})'
            )
            print(
                f'  Current NCBITaxon:{taxon.ncbi_id} → "{ncbi_val.ncbi_current_label}" (species level)'
            )
            print(f'  Strain search: No NCBITaxon entry for "{strain_info.strain_designation}"')
            if gtdb_info.found and gtdb_info.lineage:
                print(f"  GTDB: {gtdb_info.lineage}")
            print("  STATUS: ACCEPTABLE - Use species ID, document strain in notes")
    ...
    if wrong_level:
        print(
            f"{Colors.BOLD}{Colors.RED}C. WRONG TAXONOMY LEVEL ({len(wrong_level)}){Colors.RESET}"
        )
        print(f"{'-'*80}")
        for taxon, ncbi_val, gtdb_info in wrong_level:
            ...
            print(
                f'  Literature: "{taxon.preferred_term}" (strain: {strain_info.strain_designation})'
            )
            print(
                f'  Current NCBITaxon:{taxon.ncbi_id} → "{ncbi_val.ncbi_current_label}" (species) ✓'
            )
            if ncbi_val.strain_level_id:
                print(
                    f'  Strain NCBITaxon:{ncbi_val.strain_level_id} → "{ncbi_val.strain_level_label}" EXISTS!'
                )
                print(
                    f"  {Colors.BOLD}ACTION: Update to NCBITaxon:{ncbi_val.strain_level_id} for strain resolution{Colors.RESET}"
                )
            print(f"  File: {taxon.file_name}")
            print()
    ...
    if strain_not_in_db:
        print(
            f"{Colors.BOLD}{Colors.CYAN}D. STRAIN NOT IN DATABASES ({len(strain_not_in_db)}){Colors.RESET}"
        )
        print(f"{'-'*80}")
        for taxon, ncbi_val, gtdb_info in strain_not_in_db:
            ...
            if ncbi_val.species_level_id:
                print(
                    f'  NCBITaxon species: {ncbi_val.species_level_id} "{ncbi_val.species_level_label}" (found)'
                )
            print(f'  NCBITaxon strain: Not found for "{strain_info.strain_designation}"')
            print("  GTDB: Not found")
            print(
                f"  RECOMMENDATION: Use NCBITaxon:{ncbi_val.species_level_id} (species), add notes:"
            )
            print(
                f'    "Specific strain: {strain_info.strain_designation} (not in NCBITaxon/GTDB)"'
            )
            print(f"  File: {taxon.file_name}")
            print()
    ...
    if mismatches:
        print(
            f"{Colors.BOLD}{Colors.RED}E. NCBITAXON ID MISMATCHES ({len(mismatches)}){Colors.RESET}"
        )
        print(f"{'-'*80}")
        for taxon, ncbi_val, gtdb_info in mismatches:
            print(f"{Colors.RED}✗{Colors.RESET} {taxon.preferred_term}")
            print(
                f'  Literature: "{taxon.preferred_term}" {Colors.YELLOW}(SOURCE OF TRUTH){Colors.RESET}'
            )
            print(
                f'  Current NCBITaxon:{taxon.ncbi_id} → "{ncbi_val.ncbi_current_label}" {Colors.RED}✗{Colors.RESET}'
            )
            if ncbi_val.suggested_ncbi_id:
                print(
                    f'  Suggested NCBITaxon:{ncbi_val.suggested_ncbi_id} → "{ncbi_val.suggested_ncbi_label}"'
                )
            if gtdb_info.found and gtdb_info.lineage:
                print(f"  GTDB: {gtdb_info.lineage}")
            print(f"  File: {taxon.file_name}")
...
def write_output_files(
    all_results: list[tuple[TaxonInfo, NCBIValidation, GTDBInfo]], output_dir: Path
):
    ...
    with open(corrections_file, "w", newline="") as f:
        ...
        for taxon, ncbi_val, gtdb_info in all_results:
            ...
            if ncbi_val.suggested_ncbi_id and ncbi_val.suggested_ncbi_id != taxon.ncbi_id:
                ...
                ncbi_val.recommendation == "USE_STRAIN"
                and ncbi_val.strain_level_id != taxon.ncbi_id
            ):
            ...
            taxonomy_level = (
                "STRAIN"
                if strain_info.strain_designation and ncbi_val.strain_level_id
                else "SPECIES"
            )
...
def main():
    ...
    parser.add_argument(
        "--gtdb-path", type=Path, default=None, help="Path to local GTDB database (optional)"
    )
    parser.add_argument(
        "--skip-gtdb", action="store_true", help="Skip GTDB validation (only validate against NCBI)"
    )
    parser.add_argument(
        "--use-gtdb",
        action="store_true",
    ...
    print(
        f"{Colors.CYAN}Hierarchy: Literature Strain → NCBI Strain → NCBI Species → GTDB{Colors.RESET}\n"
    )

    # Check dependencies
    if not OAKLIB_AVAILABLE:
        print(
            f"{Colors.RED}Error: oaklib is required. Install with: pip install oaklib{Colors.RESET}"
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            f"{Colors.YELLOW}You may need to run: runoak -i sqlite:obo:ncbitaxon info{Colors.RESET}"
        )
        sys.exit(1)
    ...
    if args.use_gtdb:
        try:
            ...
            print(
                f"{Colors.YELLOW}Run: python scripts/gtdb_integration.py --download --load{Colors.RESET}\n"
            )
            gtdb_conn = None
    ...
    print(
        f"{Colors.CYAN}Validating against NCBITaxonomy with strain-level resolution...{Colors.RESET}"
    )
    results = []
    ...
    if not args.skip_gtdb and not gtdb_conn:
        print(
            f"{Colors.YELLOW}Note: GTDB validation not performed (database not loaded).{Colors.RESET}"
        )
        ...
        print(
            f"{Colors.YELLOW}  3. python scripts/compare_ncbi_gtdb_taxonomy.py --use-gtdb{Colors.RESET}\n"
        )
