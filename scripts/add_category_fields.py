...
def determine_community_category(name: str, description: str) -> str:
    ...
    if any(
        keyword in text for keyword in ["richmond mine", "tinto river", "acid mine drainage", "amd"]
    ):
        return "AMD"
    ...
    if any(
        keyword in text
        for keyword in ["rhizosphere", "root", "arabidopsis", "wheat", "sorghum", "maize", "lotus"]
    ):
        return "RHIZOSPHERE"
    ...
    if any(
        keyword in text
        for keyword in ["extreme", "thermophilic", "halophilic", "deep subsurface", "acidophil"]
    ):
...
def process_yaml_file(file_path: Path) -> tuple[bool, str]:
    ...
    try:
        ...
        desc_match = re.search(
            r"^description:\s*>(.+?)(?=^[a-z_]+:)", content, re.MULTILINE | re.DOTALL
        )
        if not desc_match:
