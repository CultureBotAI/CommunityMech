...
class KGMStrainLookup:
    ...
    def __init__(
        ...
        if needs_loading:
            ...
            count = self.conn.execute("SELECT COUNT(*) FROM ncbitaxon").fetchone()[0]
            print(
                f"{Colors.GREEN}Using existing database with {count:,} NCBITaxon records{Colors.RESET}"
            )
    ...
    def _load_kgm_data(self):
        ...
        try:
            ...
            """)
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_ncbi_name_lower ON ncbitaxon(name_lower)"
            )
        except Exception as e:
            print(
                f"{Colors.YELLOW}  Warning: Could not create normalized name index: {e}{Colors.RESET}"
            )
        ...
        print(
            f"{Colors.GREEN}Loaded {count:,} NCBITaxon records in {elapsed:.1f} seconds{Colors.RESET}"
        )
    ...
    def extract_strain_info(self, organism_name: str, notes: str = "") -> StrainInfo:
        ...
        # No strain found
        return StrainInfo(
            species_name=organism_name.strip(), strain_designation=None, original_term=organism_name
        )
    ...
    def search_by_synonym(self, organism_name: str) -> list[KGMMatch]:
        ...
        """,
            [organism_name, f"{organism_name}|%", f"%|{organism_name}|%", f"%|{organism_name}"],
        ).fetchall()
    ...
    def search_by_fuzzy_name(
        self, organism_name: str, include_synonyms=True, limit=10
    ) -> list[KGMMatch]:
        """
    ...
    def get_taxonomy_hierarchy(self, ncbitaxon_id: str) -> dict[str, str]:
        ...
        hierarchy = {"id": result[0], "name": result[1], "category": result[2], "xref": result[3]}
    ...
    def resolve_strain(
        self, preferred_term: str, current_ncbi_id: str = None
        ...
        if strain_info.strain_designation:
            ...
            kgm_matches = self.search_strain_specific(
                strain_info.species_name, strain_info.strain_designation
            )
        else:
        ...
        if not kgm_matches:
            ...
            if current_ncbi_id and match_id == current_ncbi_id:
                ...
                if current_ncbi_id:
                    reason = (
                        f"kg-microbe suggests NCBITaxon:{match_id} (currently {current_ncbi_id})"
                    )
                else:
    ...
    def validate_community_taxa(
        ...
        for i, taxon in enumerate(all_taxa, 1):
            ...
            result_entry = {"taxon": taxon, "resolution": resolution}
    ...
    def generate_corrections_report(
        ...
        with open(corrections_file, "w", newline="") as f:
            ...
            for category, entries in results.items():
                for entry in entries:
                    ...
                    kgm_confidence = (
                        resolution.kgm_matches[0].confidence if resolution.kgm_matches else ""
                    )
        ...
        with open(report_file, "w") as f:
            ...
            f.write(
                f"Perfect matches (ID matches kg-microbe): {perfect} ({stats['perfect_match_pct']}%)\n"
            )
            f.write(
                f"Name matches but different ID: {diff_id} ({stats['name_match_diff_id_pct']}%)\n"
            )
            ...
            for entry in results["perfect_match"][:10]:
                ...
                f.write(
                    f"    kg-microbe: {resolution.kgm_matches[0].name if resolution.kgm_matches else 'N/A'}\n"
                )
            ...
            for entry in results["name_match_diff_id"]:
                ...
                f.write(
                    f"    kg-microbe name: {resolution.kgm_matches[0].name if resolution.kgm_matches else 'N/A'}\n"
                )
            ...
            for entry in results["not_found_in_kgm"]:
                ...
                f.write(f"  {taxon.preferred_term}\n")
                f.write(f"    Current ID: NCBITaxon:{taxon.ncbi_id}\n")
                f.write("    Status: Not found in kg-microbe database\n")
                f.write(f"    File: {taxon.file_name}\n\n")
        ...
        print(
            f"{Colors.GREEN}Perfect matches: {perfect}/{total} ({stats['perfect_match_pct']}%){Colors.RESET}"
        )
        print(
            f"{Colors.YELLOW}Different IDs: {diff_id}/{total} ({stats['name_match_diff_id_pct']}%){Colors.RESET}"
        )
        print(
            f"{Colors.MAGENTA}Needs review: {review}/{total} ({stats['review_pct']}%){Colors.RESET}"
        )
        print(
            f"{Colors.RED}Not found: {not_found}/{total} ({stats['not_found_pct']}%){Colors.RESET}"
        )
...
def main():
    parser = argparse.ArgumentParser(description="KG-Microbe strain lookup and validation system")
    parser.add_argument(
        "--load", action="store_true", help="Load/reload kg-microbe data into DuckDB"
    )
    parser.add_argument("--search", type=str, help="Search for organism by name")
    parser.add_argument(
        "--strain",
        type=str,
        nargs=2,
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate all community YAMLs against kg-microbe"
    )
    parser.add_argument(
        "--report", action="store_true", help="Generate comprehensive correction report"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    lookup = KGMStrainLookup(
        kgm_data_dir=args.kgm_data_dir, db_path=args.db_path, force_reload=args.load
    )
