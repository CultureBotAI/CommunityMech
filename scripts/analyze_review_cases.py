    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        for cat_key, cat_info in category_info.items():
            ...
            for i, item in enumerate(cases, 1):
                ...
                f.write(
                    f"   Current ID: NCBITaxon:{case['current_id']} ({case['current_label']})\n"
                )
                f.write(
                    f"   Recommended ID: NCBITaxon:{case['recommended_id']} ({case['recommended_label']})\n"
                )
                ...
                if cat_key == "taxonomy_rename_phylum":
                    f.write(
                        f"   Old name: {details['old_name']} → New name: {details['new_name']}\n"
                    )
                    f.write(f"   Type: {details['rename_type']}\n")
                    f.write(
                        f"   Action: Literature may use '{details['old_name']}' - document this\n"
                    )
                    ...
                    f.write(
                        f"   PROBLEM: Expected '{details['expected']}' but ID points to '{details['got']}'\n"
                    )
                    f.write("   Action: URGENT - Search NCBI for correct ID\n")
...
def generate_high_priority_report(categorized, output_path):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        for i, (category, item) in enumerate(high_priority_cases, 1):
            ...
            f.write(
                f"   Recommended: NCBITaxon:{case['recommended_id']} ({case['recommended_label']})\n"
            )
            ...
            if category == "wrong_id":
                ...
                f.write("   Action: Search NCBI Taxonomy database for correct ID\n")
