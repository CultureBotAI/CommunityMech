...
class TaxonomyCorrector:
    ...
    def load_corrections(self):
        ...
        print(
            f"{Colors.CYAN}Loading corrections from {self.corrections_file.name}...{Colors.RESET}"
        )
        ...
        for taxon_entry in data["taxonomy"]:
            ...
            if preferred_term == correction["preferred_term"]:
                ...
                if current_id_stripped == expected_id_stripped:
                    ...
                    print(
                        f"  {Colors.YELLOW}⊙{Colors.RESET} {preferred_term}: ID mismatch (expected {expected_id_stripped}, found {current_id_stripped})"
                    )
...
def main():
    ...
    if not corrections_file.exists():
        ...
        print(
            f"{Colors.CYAN}Run 'poetry run python scripts/compare_ncbi_gtdb_taxonomy.py' first{Colors.RESET}"
        )
        return
    ...
    print("  1. Validate updated files: just validate kb/communities/*.yaml")
    print("  2. Review changes: git diff kb/communities/")
    print("  3. Test browser: just gen-browser && just serve")
