...
class EvidenceCurator:
    ...
    def __init__(self, use_pdf_fallback: bool = True, auto_fix: bool = False):
        self.fetcher = EnhancedLiteratureFetcher(
            cache_dir=".literature_cache", use_fallback_pdf=use_pdf_fallback
        )
        self.auto_fix = auto_fix
        self.issues = []
        self.stats = defaultdict(int)

    def validate_reference_format(self, reference: str) -> tuple[bool, str | None]:
    ...
    def fetch_and_validate_evidence(
        self, reference: str, snippet: str, organism: str = None, fetch_pdf: bool = False
    ) -> dict:
        """
    ...
    def _extract_best_snippet(
        self, text: str, organism: str = None, current_snippet: str = None
    ) -> str | None:
        ...
        if current_snippet:
            keywords = [
                w
                for w in re.findall(r"\b\w{4,}\b", current_snippet.lower())
    ...
    def audit_community_yaml(
        self, yaml_path: Path, fetch_pdfs: bool = False
    ) -> list[EvidenceIssue]:
    ...
    def _audit_evidence_item(
        self, evidence: dict, filename: str, context: str, organism: str, fetch_pdf: bool
    ) -> list[EvidenceIssue]:
        ...
        if snippet and ref_valid:
            ...
            validation = self.fetch_and_validate_evidence(reference, snippet, organism, fetch_pdf)
            ...
            if not validation["abstract_fetched"]:
                ...
                issues.append(
                    EvidenceIssue(
                        ...
                        current_value=snippet[:100],
                        suggested_fix=(
                            validation["suggested_snippet"][:150]
                            if validation["suggested_snippet"]
                            else None
                        ),
                        severity="ERROR",
                    )
    ...
    def generate_report(self, issues: list[EvidenceIssue], output_path: Path):
        ...
        with open(output_path, "w") as f:
            ...
            f.write(
                f"  - Invalid reference format: {self.stats.get('invalid_reference_format', 0)}\n"
            )
            f.write(
                f"  - Missing evidence source: {self.stats.get('missing_evidence_source', 0)}\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--auto-fix", action="store_true", help="Automatically apply fixes where possible"
    )
    parser.add_argument("--quick", action="store_true", help="Quick audit (skip PDF fetching)")
    args = parser.parse_args()

    curator = EvidenceCurator(use_pdf_fallback=not args.quick, auto_fix=args.auto_fix)
