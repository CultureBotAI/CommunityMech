...
REQUIRED_JOBS = {
    ...
    "docs-current.yaml": {"docs-current": "docs-current"},
...
def test_queue_checks_cannot_post_pr_comments_or_request_paid_repairs():
    document = _load("network-quality.yml")
    comments = next(
        step
        for step in document["jobs"]["audit-network"]["steps"]
        if step.get("name") == "Comment on PR with findings"
    )
    assert comments["if"] == "github.event_name == 'pull_request'"
    assert document["jobs"]["suggest-repairs"]["if"] == (
        "${{ !cancelled() && github.event_name == 'workflow_dispatch' && inputs.suggest_repairs }}"
    )
